...
// API endpoint to fetch historical moments ONLY from DeepSeek
app.post('/api/history', async (req, res) => {
  ...
  if (!deepseekApiKey) {
    return res.status(400).json({ 
      error: '未检测到 DEEPSEEK_API_KEY。请在页面右上角点击「Settings」->「Secrets」添加 DEEPSEEK_API_KEY 环境变量并填入您的 Key。' 
    });
  }
  ...
  try {
    ...
    console.log(`Attempting DeepSeek query using endpoint: ${endpoint}`);
  ...
  { 
    ...
    if (Array.isArray(parsed) && parsed.length === 5) {
      ...
      if (validEvents.length === 5) {
        console.log("Successfully fetched from DeepSeek!");
        return res.json(validEvents);
      }
    }
    throw new Error("格式解析不匹配，请重新尝试。");
  } catch (dsError: any) {
    console.warn("DeepSeek query failed:", dsError.response?.data || dsError.message);
    const errorMsg = dsError.response?.data?.error?.message || dsError.message || "未知原因";
    return res.status(500).json({ error: `DeepSeek 调用失败: ${errorMsg}` });
  }
});

// New API endpoint to generate historical event explanations strictly via DeepSeek
...
请以简体中文书写，字数在 250 到 350 字之间。排版精美，使用换行分割段落。`;
  ...
  try {
    ...
    console.log(`Attempting DeepSeek explanation using endpoint: ${endpoint}`);
    ...
    if (explanation) {
      console.log("Successfully generated explanation via DeepSeek!");
      return res.json({ explanation: toSimplified(explanation) });
    }
    throw new Error("生成的内容为空");
  } catch (dsError: any) {
    console.warn("DeepSeek explain failed:", dsError.response?.data || dsError.message);
    const errorMsg = dsError.response?.data?.error?.message || dsError.message || "未知原因";
    return res.status(500).json({ error: `DeepSeek 解析失败: ${errorMsg}` });
  }
});
